...
# to do: order path as (a,b) (b,c) (c,d) instead of (a,b) (c,b), ...
def shortest_path(graph, a, b, visited = []):
	possible_steps = []
	possible_paths = []
	...
	for edge in possible_steps:
		if b in edge:
			...
			if not (new_a in visited):
				possible_paths += [[edge] + shortest_path(graph, new_a, b, visited + [a])]
	...
	path = min(possible_paths, key=len)
	for i in range(1, len(path)):
		if path[i][0] != path[i-1][0]: path[i] = path[i][::-1] #reverse it
